...
public class BuildContexts<T extends ExecutionCommandSource<T>> {
    ...
    public static final DynamicCommandExceptionType ERROR_FORK_LIMIT_REACHED = new DynamicCommandExceptionType((p_311924_) -> {
        return CustomComponent.translatableEscape("command.forkLimit", p_311924_);
    });
    ...
    protected void execute(T p_309755_, List<T> p_310231_, ExecutionContext<T> p_311779_, Frame p_313162_, ChainModifiers p_310618_) {
        ...
        if (contextchain.getStage() != ContextChain.Stage.EXECUTE) {
            ...
            try {
                for(int i = p_311779_.forkLimit(); contextchain.getStage() != ContextChain.Stage.EXECUTE; contextchain = contextchain.nextStage()) {
    ...
    protected void traceCommandStart(ExecutionContext<T> p_311913_, Frame p_312311_) {
        ServerFunctionManager.TraceCallbacks tracecallbacks = p_311913_.tracer();
